...
import com.my.rental.config.EmbeddedKafka;
import com.my.rental.config.KafkaSseConsumer;
import com.my.rental.config.KafkaSseProducer;
...
class RentalKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/rental-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/rental-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/rental-kafka/unregister"));
                return;
            }
        }
