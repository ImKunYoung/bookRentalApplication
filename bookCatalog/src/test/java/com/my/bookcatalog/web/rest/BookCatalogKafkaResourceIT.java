...
import com.my.bookcatalog.config.EmbeddedKafka;
import com.my.bookcatalog.config.KafkaSseConsumer;
import com.my.bookcatalog.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class BookCatalogKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/book-catalog-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/book-catalog-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/book-catalog-kafka/unregister"));
                return;
            }
        }
