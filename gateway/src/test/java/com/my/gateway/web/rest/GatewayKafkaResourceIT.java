...
import com.my.gateway.config.EmbeddedKafka;
import com.my.gateway.config.KafkaSseConsumer;
import com.my.gateway.config.KafkaSseProducer;
...
class GatewayKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/gateway-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
